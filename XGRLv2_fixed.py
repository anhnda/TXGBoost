"""
REINFORCEMENT LEARNING V2 FIXED: RNN Policy Network → XGBoost Judge

Fixes for XGRLv2.py training dynamics issues:
1. Remove harmful supervised pretraining (or use XGBoost-aligned pretraining)
2. Increase learning rate (0.0005 instead of 0.0001)
3. Remove non-stationary global rewards (val_aupr)
4. Increase entropy bonus (0.01 instead of 0.001)
5. Remove aggressive temperature annealing
6. Keep enriched features: [Static + Last + Mean + Std + Z]

This should work because the features are fine - the training setup was the problem!
"""
...
# ==============================================================================
# 4. Feature Extraction with Enriched Temporal Features
# ==============================================================================
...
def train_policy_fixed_rl(
    ...
    """
    FIXED RL Training:
    1. Higher learning rate (0.0005 vs 0.0001)
    2. No global reward (remove val_aupr)
    3. Higher entropy (0.01 vs 0.001)
    4. No aggressive temperature annealing
    """

    optimizer = torch.optim.Adam(policy_net.parameters(), lr=0.0005)  # FIX 1: Higher LR
    ...
    print("  [FIXED RL] Training with corrected hyperparameters...")

    xgb_model = None

    for epoch in range(epochs):
        # FIX 4: Gentler temperature annealing (or none)
        temperature = max(0.5, 1.0 - epoch / (epochs * 2.0))  # Slower: 1.0 → 0.5
        ...
        # FIX 3: Stronger entropy bonus
        entropy_bonus = 0.01 * log_probs_train.mean()  # 10x stronger than XGRLv2
...
def main():
    print("="*80)
    print("RL POLICY V2 FIXED with XGBoost")
    print("="*80)
    ...
    for fold, (train_full, test_p) in enumerate(trainTestPatients(patients, seed=xseed)):
        ...
        # Use same capacity as XGRLv2 but WITHOUT pretraining
        latent_dim = 28
        ...
        # NO PRETRAINING - Start RL directly from random initialization
        print("  [Skipping pretraining - starting RL from scratch]")

        # FIXED RL Training
        policy_net = train_policy_fixed_rl(
            policy_net,
            train_loader,
            val_loader,
            xgb_params,
    ...
    ax1.set_title("RL Policy V2 Fixed + XGBoost", fontsize=14, fontweight='bold')
    ax1.legend(loc="lower right")
    ax1.grid(alpha=0.3)

    plt.tight_layout()
    plt.savefig("result/xgrl_v2_fixed.png", dpi=300)
    print("\nPlot saved to result/xgrl_v2_fixed.png")
