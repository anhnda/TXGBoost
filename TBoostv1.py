"""
ENHANCED TRIPLE HYBRID MODEL v1: [Learned Trend + Last Value + Static] -> XGBoost
vs
...
# ==============================================================================
# 1. Helpers: Encoder & Gated Head & Global Stats Computer
# ==============================================================================
...
# ==============================================================================
# 2. Enhanced Dataset (Returns Temporal, Label, Original Static AND Enhanced Static)
# ==============================================================================

class EnhancedHybridDatasetV1(Dataset):
    """
    Dataset that returns both original and enhanced static features.
    - Enhanced static (with global stats) is used for RNN training
    - Original static is used for final XGBoost feature concatenation
    """
    def __init__(self, patients, feature_names, static_encoder, normalization_stats=None):
        ...
        for patient in patient_list:
            ...
            self.original_static_data.append(torch.tensor(s_vec, dtype=torch.float32))
            enhanced_static = np.concatenate([s_vec, global_stats])
            self.enhanced_static_data.append(torch.tensor(enhanced_static, dtype=torch.float32))
    ...
    def __getitem__(self, idx):
        # Return temporal data, label, enhanced static (for RNN), original static (for XGBoost)
        return self.data[idx], self.labels[idx], self.enhanced_static_data[idx], self.original_static_data[idx]

def enhanced_collate_fn_v1(batch):
    """Collate function that handles both enhanced and original static features"""
    data_list, label_list, enhanced_static_list, original_static_list = zip(*batch)
    ...
    return (temporal_batch,
            ...
            torch.stack(enhanced_static_list),
            torch.stack(original_static_list))
...
def train_rnn_extractor(model, train_loader, val_loader, criterion, optimizer, epochs=50):
    ...
    for epoch in range(epochs):
        ...
        for t_data, labels, enhanced_s_data, _ in train_loader:  # Use enhanced static for training
            labels = labels.to(DEVICE)
            enhanced_s_data = enhanced_s_data.to(DEVICE)
            h = model(t_data)
            combined = torch.cat([h, enhanced_s_data], dim=1)
        ...
        if (epoch+1) % 5 == 0:
            ...
            all_preds, all_lbls = [], []
            with torch.no_grad():
                for t_data, labels, enhanced_s_data, _ in val_loader:
                    enhanced_s_data = enhanced_s_data.to(DEVICE)
                    h = model(t_data)
                    combined = torch.cat([h, enhanced_s_data], dim=1)
                    preds = temp_head(combined).squeeze(-1)
...
# ==============================================================================
# 4. Triple Feature Extraction (v1: Uses Original Static for XGBoost)
# ==============================================================================

def get_triple_features_v1(model, loader):
    """
    ...
    with torch.no_grad():
        for t_data, labels, _, original_s_data in loader:  # Use original static
            # 1. Get RNN Embedding (128 dims) - trained with enhanced context
            h = model(t_data).cpu().numpy()

            # 2. Get Original Static Features (23 dims) for XGBoost
            s = original_s_data.numpy()
            ...
            # 4. TRIPLE CONCATENATION: [Last Values + Original Static + RNN]
...
def main():
    print("="*80)
    print("TBoostv1: RNN Enhanced with Global Stats, XGBoost with Original Features")
    print("="*80)
    ...
    for fold, (train_full, test_p) in enumerate(trainTestPatients(patients,seed=xseed)):
        ...
        # 1. Enhanced Hybrid Data Setup (v1)
        train_ds = EnhancedHybridDatasetV1(train_p, temporal_feats, encoder)
        stats = train_ds.get_normalization_stats()
        val_ds = EnhancedHybridDatasetV1(val_p, temporal_feats, encoder, stats)
        test_ds = EnhancedHybridDatasetV1(test_p_list, temporal_feats, encoder, stats)

        train_loader = DataLoader(train_ds, batch_size=32, shuffle=True, collate_fn=enhanced_collate_fn_v1)
        val_loader = DataLoader(val_ds, batch_size=32, shuffle=False, collate_fn=enhanced_collate_fn_v1)
        test_loader = DataLoader(test_ds, batch_size=32, shuffle=False, collate_fn=enhanced_collate_fn_v1)

        # 2. Stage 1: RNN Training (with enhanced static context)
        ...
        # 3. Stage 2: Triple Feature Fusion (v1 - uses original static)
        print("  [Stage 2] Extracting Triple Features (with original static)...")
        X_train, y_train = get_triple_features_v1(rnn, train_loader)
        X_val, y_val = get_triple_features_v1(rnn, val_loader)
        X_test, y_test = get_triple_features_v1(rnn, test_loader)

        print(f"    Feature dimensions: {X_train.shape[1]} (25 Last + 23 Original Static + 128 RNN)")
        ...
        print(f"  Fold {fold} Results -> TBoostv1: {metrics_hybrid['auc'][-1]:.3f} vs Baseline: {metrics_base['auc'][-1]:.3f}")
    ...
    ax1.set_title("TBoostv1 (RNN+Last+Static)")
    ax2.set_title("Baseline (Last+Static)")
    plt.tight_layout()
    plt.savefig("result/tboostv1_vs_baseline.png", dpi=300)
    print("\nPlot saved to result/tboostv1_vs_baseline.png")
    ...
    def print_stat(name, h_metrics, b_metrics):
        ...
        print(f"{name:15s} | TBoostv1: {h_mean:.4f} ± {h_std:.4f}  vs  Baseline: {b_mean:.4f} ± {b_std:.4f}")
