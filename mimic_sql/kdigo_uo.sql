    ...
    SELECT ie.stay_id,
        ...
        COALESCE(
            CAST(
                EXTRACT(
                    EPOCH
                    FROM charttime - LAG(charttime) OVER (
...
,
        ...
        urineoutput
    FROM icustays AS ie
        INNER JOIN urine_output AS uo ON ie.stay_id = uo.stay_id
),
uo_stg2 AS (
    SELECT stay_id,
        charttime,
        ...
        SUM(urineoutput) OVER (
            ...
            ORDER BY seconds_since_admit NULLS FIRST RANGE BETWEEN 21600 PRECEDING AND CURRENT ROW
        ) AS urineoutput_6hr,
        SUM(urineoutput) OVER (
            PARTITION BY stay_id
            ORDER BY seconds_since_admit NULLS FIRST RANGE BETWEEN 43200 PRECEDING AND CURRENT ROW
        ) AS urineoutput_12hr,
        SUM(urineoutput) OVER (
            PARTITION BY stay_id
            ORDER BY seconds_since_admit NULLS FIRST RANGE BETWEEN 86400 PRECEDING AND CURRENT ROW
        ) AS urineoutput_24hr,
