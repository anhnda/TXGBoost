...
WITH base AS (
  SELECT subject_id,
    ...
    MAX(
      CASE
        WHEN ce.itemid = 223901 THEN ce.valuenum
        ELSE NULL
      END
    ) AS gcsmotor,
    MAX(
    ...
    MAX(
      CASE
        WHEN ce.itemid = 220739 THEN ce.valuenum
        ELSE NULL
      END
    ) AS gcseyes,
    ...
    MAX(
      CASE
        WHEN ce.itemid = 223900
        AND ce.value = 'No Response-ETT' THEN 1
        ELSE 0
      END
    ) AS endotrachflag,
    ROW_NUMBER() OVER (
      PARTITION BY ce.stay_id
      ORDER BY ce.charttime ASC NULLS FIRST
    ) AS rn
...
gcs AS (
  ...
  FROM base AS b
    ...
    LEFT JOIN base AS b2 ON b.stay_id = b2.stay_id
    AND b.rn = b2.rn + 1
