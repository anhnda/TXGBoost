...
WITH vaso_stg AS (
  SELECT ie.stay_id,
    ...
    vaso_rate AS rate
  FROM icustays AS ie
    INNER JOIN norepinephrine AS mv ON ie.stay_id = mv.stay_id
  ...
  SELECT ie.stay_id,
    ...
    vaso_rate AS rate
  FROM icustays AS ie
    INNER JOIN epinephrine AS mv ON ie.stay_id = mv.stay_id
    AND mv.starttime >= ie.intime - INTERVAL '6 HOUR'
    AND mv.starttime <= ie.intime + INTERVAL '1 DAY'
  UNION ALL
  SELECT ie.stay_id,
    ...
    vaso_rate AS rate
  FROM icustays AS ie
    INNER JOIN dobutamine AS mv ON ie.stay_id = mv.stay_id
    AND mv.starttime >= ie.intime - INTERVAL '6 HOUR'
    AND mv.starttime <= ie.intime + INTERVAL '1 DAY'
  UNION ALL
  SELECT ie.stay_id,
    ...
    vaso_rate AS rate
  FROM icustays AS ie
    INNER JOIN dopamine AS mv ON ie.stay_id = mv.stay_id
    AND mv.starttime >= ie.intime - INTERVAL '6 HOUR'
    AND mv.starttime <= ie.intime + INTERVAL '1 DAY'
),
vaso_mv AS (
  SELECT ie.stay_id,
    MAX(
      CASE
        WHEN treatment = 'norepinephrine' THEN rate
        ELSE NULL
      END
    ) AS rate_norepinephrine,
    MAX(
      CASE
        WHEN treatment = 'epinephrine' THEN rate
        ELSE NULL
      END
    ) AS rate_epinephrine,
    MAX(
      CASE
        WHEN treatment = 'dopamine' THEN rate
        ELSE NULL
      END
    ) AS rate_dopamine,
    MAX(
      CASE
        WHEN treatment = 'dobutamine' THEN rate
        ELSE NULL
      END
    ) AS rate_dobutamine
  FROM icustays AS ie
    LEFT JOIN vaso_stg AS v ON ie.stay_id = v.stay_id
  GROUP BY ie.stay_id
),
pafi1 AS (
  ...
  SELECT ie.stay_id,
    ...
    END AS isvent
  FROM icustays AS ie
    LEFT JOIN bg AS bg ON ie.subject_id = bg.subject_id
...
scorecomp AS (
  SELECT ie.stay_id,
    ...
    gcs.gcs_min
  FROM icustays AS ie
    LEFT JOIN vaso_mv AS mv ON ie.stay_id = mv.stay_id
    LEFT JOIN pafi2 AS pf ON ie.stay_id = pf.stay_id
    LEFT JOIN first_day_vitalsign AS v ON ie.stay_id = v.stay_id
    LEFT JOIN first_day_lab AS l ON ie.stay_id = l.stay_id
...
SELECT ie.subject_id,
  ...
  renal
FROM icustays AS ie
  LEFT JOIN scorecalc AS s ON ie.stay_id = s.stay_id