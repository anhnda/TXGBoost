...
WITH wt_stg AS (
    SELECT c.stay_id,
        c.charttime,
        CASE
            WHEN c.itemid = 226512 THEN 'admit'
            ELSE 'daily'
        END AS weight_type,
        /* TODO: eliminate obvious outliers if there is a reasonable weight */
        c.valuenum AS weight
    FROM chartevents AS c
    WHERE NOT c.valuenum IS NULL
        AND c.itemid IN (
            226512
            /* Admit Wt */
,
...
wt_stg2 AS (
    SELECT wt_stg1.stay_id,
        ...
        wt_stg1.weight
    FROM wt_stg1
        INNER JOIN icustays AS ie ON ie.stay_id = wt_stg1.stay_id
),
wt_stg3 AS (
    SELECT stay_id,
        intime,
        ...
        COALESCE(
            LEAD(starttime) OVER (
                PARTITION BY stay_id
                ORDER BY starttime NULLS FIRST
            ),
        ...
        INNER JOIN (
            SELECT wt1.stay_id,
                ...
                ROW_NUMBER() OVER (
                    ...
                    ORDER BY wt1.starttime NULLS FIRST
                ) AS rn
            FROM wt1
        ) AS wt ON ie.stay_id = wt.stay_id
        AND wt.rn = 1
