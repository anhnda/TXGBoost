...
uo_stg AS (
    SELECT uo.stay_id,
        ...
        END AS aki_stage_uo
    FROM kdigo_uo AS uo
        INNER JOIN icustays AS ie ON uo.stay_id = ie.stay_id
),
crrt_stg AS (
    SELECT stay_id,
        charttime,
        CASE
            WHEN NOT charttime IS NULL THEN 3
            ELSE NULL
        END AS aki_stage_crrt
    FROM crrt
    WHERE NOT crrt_mode IS NULL
),
tm_stg AS (
    SELECT stay_id,
        charttime
...
SELECT ie.subject_id,
    ...
    MAX(
        ...
        ORDER BY EXTRACT(
                EPOCH
                FROM tm.charttime - ie.intime
            ) / 1.0 NULLS FIRST RANGE BETWEEN 21600 PRECEDING AND CURRENT ROW
    ) AS aki_stage_smoothed
FROM icustays AS ie
    /* get all possible charttimes as listed in tm_stg */
    LEFT JOIN tm_stg AS tm ON ie.stay_id = tm.stay_id
    LEFT JOIN cr_stg AS cr ON ie.stay_id = cr.stay_id
    AND tm.charttime = cr.charttime
    LEFT JOIN uo_stg AS uo ON ie.stay_id = uo.stay_id
    AND tm.charttime = uo.charttime
    LEFT JOIN crrt_stg AS crrt ON ie.stay_id = crrt.stay_id
