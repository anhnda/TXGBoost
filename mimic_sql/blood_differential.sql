  ...
  SELECT MAX(subject_id) AS subject_id,
    ...
    MAX(
      CASE
        ...
        WHEN itemid = 51199 THEN CAST(valuenum AS DOUBLE PRECISION) / 1000.0
        ELSE NULL
      END
    ) AS eosinophils_abs,
    ...
    MAX(
      CASE
        ...
        WHEN itemid = 52769 THEN CAST(valuenum AS DOUBLE PRECISION) / 1000.0
        ELSE NULL
      END
    ) AS lymphocytes_abs,
    ...
    MAX(
      CASE
        ...
        WHEN itemid = 51253 THEN CAST(valuenum AS DOUBLE PRECISION) / 1000.0
        ELSE NULL
      END
    ) AS monocytes_abs,
    MAX(
    ...
    MAX(
      CASE
        WHEN itemid = 51218 THEN CAST(valuenum AS DOUBLE PRECISION) / 1000.0
        ELSE NULL
      END
    ) AS granulocytes_abs,
    ...
    CASE
      ...
      AND SUM(
        CASE
          WHEN itemid IN (51146, 51200, 51244, 51245, 51254, 51256) THEN valuenum
          ELSE NULL
        END
      ) > 0 THEN 1
      ELSE 0
  ...
  ROUND(
    CAST(
      CASE
        ...
        AND NOT basophils IS NULL
        AND impute_abs = 1 THEN CAST(basophils * wbc AS DOUBLE PRECISION) / 100
        ELSE basophils_abs
      END AS DECIMAL
    ),
    4
  ...
  ROUND(
    CAST(
      CASE
        ...
        AND NOT eosinophils IS NULL
        AND impute_abs = 1 THEN CAST(eosinophils * wbc AS DOUBLE PRECISION) / 100
  ...
  ROUND(
    CAST(
      CASE
        ...
        AND NOT lymphocytes IS NULL
        AND impute_abs = 1 THEN CAST(lymphocytes * wbc AS DOUBLE PRECISION) / 100
  ...
  ROUND(
    CAST(
      CASE
        ...
        AND NOT monocytes IS NULL
        AND impute_abs = 1 THEN CAST(monocytes * wbc AS DOUBLE PRECISION) / 100
        ELSE monocytes_abs
      END AS DECIMAL
    ),
    4
  ...
  ROUND(
    CAST(
      CASE
        ...
        AND NOT neutrophils IS NULL
        AND impute_abs = 1 THEN CAST(neutrophils * wbc AS DOUBLE PRECISION) / 100
