...
,
            ...
            OR ventilator_mode_hamilton IN ('DuoPaP', 'NIV', 'NIV-ST') THEN 'NonInvasiveVent'
            WHEN o2_delivery_device_1 IN (
...
,
            ...
            WHEN o2_delivery_device_1 IN ('None') THEN 'None'
            ELSE NULL
        END AS ventilation_status
    FROM tm
        LEFT JOIN ventilator_setting AS vs ON tm.stay_id = vs.stay_id
        AND tm.charttime = vs.charttime
        LEFT JOIN oxygen_delivery AS od ON tm.stay_id = od.stay_id
...
vd0 AS (
    ...
    FROM vs
    WHERE NOT ventilation_status IS NULL WINDOW w AS (
            PARTITION BY stay_id
            ORDER BY charttime NULLS FIRST
        )
),
vd1 AS (
    SELECT stay_id,
        ...
        CAST(
            EXTRACT(
                EPOCH
                FROM charttime - charttime_lag
            ) / 60.0 AS DOUBLE PRECISION
        ) / 60 AS ventduration,
...
SELECT stay_id,
    ...
    MAX(
        CASE
            ...
            OR EXTRACT(
                EPOCH
                FROM charttime_lead - charttime
            ) / 3600.0 >= 14 THEN charttime
            ELSE charttime_lead
        END
    ) AS endtime,
