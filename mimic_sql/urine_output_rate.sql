...
WITH tm AS (
    ...
    FROM icustays AS ie
        INNER JOIN chartevents AS ce ON ie.stay_id = ce.stay_id
...
uo_tm AS (
    SELECT tm.stay_id,
        CASE
            WHEN LAG(charttime) OVER w IS NULL THEN EXTRACT(
                EPOCH
                FROM charttime - intime_hr
            ) / 60.0
            ELSE EXTRACT(
                EPOCH
                FROM charttime - LAG(charttime) OVER w
            ) / 60.0
        END AS tm_since_last_uo,
        uo.charttime,
        uo.urineoutput
    FROM tm
        INNER JOIN urine_output AS uo ON tm.stay_id = uo.stay_id WINDOW w AS (
...
ur_stg AS (
    SELECT io.stay_id,
        ...
        SUM(
            CASE
                WHEN EXTRACT(
                    EPOCH
                    FROM io.charttime - iosum.charttime
                ) / 3600.0 <= 5 THEN iosum.urineoutput
                ELSE NULL
            END
        ) AS urineoutput_6hr,
        CAST(
            SUM(
                CASE
                    WHEN EXTRACT(
                        EPOCH
                        FROM io.charttime - iosum.charttime
                    ) / 3600.0 <= 5 THEN iosum.tm_since_last_uo
                    ELSE NULL
                END
            ) AS DOUBLE PRECISION
        ) / 60.0 AS uo_tm_6hr,
        SUM(
            CASE
                WHEN EXTRACT(
                    EPOCH
                    FROM io.charttime - iosum.charttime
                ) / 3600.0 <= 11 THEN iosum.urineoutput
                ELSE NULL
            END
        ) AS urineoutput_12hr,
        CAST(
            SUM(
                CASE
                    WHEN EXTRACT(
                        EPOCH
                        FROM io.charttime - iosum.charttime
                    ) / 3600.0 <= 11 THEN iosum.tm_since_last_uo
                    ELSE NULL
                END
            ) AS DOUBLE PRECISION
        ) / 60.0 AS uo_tm_12hr,
    ...
    FROM uo_tm AS io
        ...
        LEFT JOIN uo_tm AS iosum ON io.stay_id = iosum.stay_id
    ...
    CASE
        WHEN uo_tm_6hr >= 6 THEN ROUND(
            CAST(
                (
                    CAST(
                        CAST(ur.urineoutput_6hr AS DOUBLE PRECISION) / wd.weight AS DOUBLE PRECISION
                    ) / uo_tm_6hr
                ) AS DECIMAL
            ),
            4
    ...
    CASE
        WHEN uo_tm_12hr >= 12 THEN ROUND(
            CAST(
                (
                    CAST(
                        CAST(ur.urineoutput_12hr AS DOUBLE PRECISION) / wd.weight AS DOUBLE PRECISION
                    ) / uo_tm_12hr
                ) AS DECIMAL
            ),
            4
    ...
    CASE
        WHEN uo_tm_24hr >= 24 THEN ROUND(
            CAST(
                (
                    CAST(
                        CAST(ur.urineoutput_24hr AS DOUBLE PRECISION) / wd.weight AS DOUBLE PRECISION
                    ) / uo_tm_24hr
                ) AS DECIMAL
            ),
            4
...
FROM ur_stg AS ur
    LEFT JOIN weight_durations AS wd ON ur.stay_id = wd.stay_id
