...
cpap AS (
  SELECT co.subject_id,
    ...
    MAX(
      CASE
        WHEN LOWER(ce.value) ~ '(cpap mask|bipap)' THEN 1
        ELSE 0
      END
    ) AS cpap
  FROM co
    INNER JOIN chartevents AS ce ON co.stay_id = ce.stay_id
    AND ce.charttime > co.starttime
    AND ce.charttime <= co.endtime
  WHERE ce.itemid = 226732
    AND LOWER(ce.value) ~ '(cpap mask|bipap)'
...
surgflag AS (
  SELECT adm.hadm_id,
    ...
    ROW_NUMBER() OVER (
      ...
      ORDER BY transfertime NULLS FIRST
    ) AS serviceorder
  FROM admissions AS adm
    LEFT JOIN services AS se ON adm.hadm_id = se.hadm_id
),
comorb AS (
  SELECT hadm_id,
    ...
    MAX(
      CASE
        WHEN icd_version = 9
        AND SUBSTR(icd_code, 1, 3) BETWEEN '042' AND '044' THEN 1
        WHEN icd_version = 10
        AND SUBSTR(icd_code, 1, 3) BETWEEN 'B20' AND 'B22' THEN 1
        WHEN icd_version = 10
        AND SUBSTR(icd_code, 1, 3) = 'B24' THEN 1
        ELSE 0
      END
    ) AS aids,
    /* HIV and AIDS */
    MAX(
      CASE
        WHEN icd_version = 9 THEN CASE
          WHEN SUBSTR(icd_code, 1, 5) BETWEEN '20000' AND '20238' THEN 1
          WHEN SUBSTR(icd_code, 1, 5) BETWEEN '20240' AND '20248' THEN 1
          WHEN SUBSTR(icd_code, 1, 5) BETWEEN '20250' AND '20302' THEN 1
          WHEN SUBSTR(icd_code, 1, 5) BETWEEN '20310' AND '20312' THEN 1
          WHEN SUBSTR(icd_code, 1, 5) BETWEEN '20302' AND '20382' THEN 1
          WHEN SUBSTR(icd_code, 1, 5) BETWEEN '20400' AND '20522' THEN 1
          WHEN SUBSTR(icd_code, 1, 5) BETWEEN '20580' AND '20702' THEN 1
          WHEN SUBSTR(icd_code, 1, 5) BETWEEN '20720' AND '20892' THEN 1
          WHEN SUBSTR(icd_code, 1, 4) IN ('2386', '2733') THEN 1
          ELSE 0
        END
        WHEN icd_version = 10
        AND SUBSTR(icd_code, 1, 3) BETWEEN 'C81' AND 'C96' THEN 1
        ELSE 0
      END
    ) AS hem,
    MAX(
      CASE
        ...
        END
        WHEN icd_version = 10
        AND SUBSTR(icd_code, 1, 3) BETWEEN 'C77' AND 'C79' THEN 1
        WHEN icd_version = 10
        AND SUBSTR(icd_code, 1, 4) = 'C800' THEN 1
        ELSE 0
      END
    ) AS mets
    /* Metastatic cancer */
...
pafi1 AS (
  ...
  SELECT co.stay_id,
    ...
    CASE
      WHEN NOT vd.stay_id IS NULL THEN 1
      ELSE 0
    END AS vent,
    CASE
      WHEN NOT cp.subject_id IS NULL THEN 1
      ELSE 0
    END AS cpap
  FROM co
    LEFT JOIN bg AS bg ON co.subject_id = bg.subject_id
    AND bg.specimen = 'ART.'
    AND bg.charttime > co.starttime
    AND bg.charttime <= co.endtime
    LEFT JOIN ventilation AS vd ON co.stay_id = vd.stay_id
    AND bg.charttime > vd.starttime
    AND bg.charttime <= vd.endtime
    AND vd.ventilation_status = 'InvasiveVent'
    LEFT JOIN cpap AS cp ON bg.subject_id = cp.subject_id
...
gcs AS (
  ...
  FROM co
    LEFT JOIN gcs AS gcs ON co.stay_id = gcs.stay_id
...
vital AS (
  ...
  FROM co
    LEFT JOIN vitalsign AS vital ON co.subject_id = vital.subject_id
...
uo AS (
  ...
  FROM co
    LEFT JOIN urine_output AS uo ON co.stay_id = uo.stay_id
...
labs AS (
  ...
  FROM co
    LEFT JOIN chemistry AS labs ON co.subject_id = labs.subject_id
...
cbc AS (
  ...
  FROM co
    LEFT JOIN complete_blood_count AS cbc ON co.subject_id = cbc.subject_id
...
enz AS (
  ...
  FROM co
    LEFT JOIN enzyme AS enz ON co.subject_id = enz.subject_id
...
cohort AS (
  SELECT ie.subject_id,
    ...
    CASE
      ...
      AND sf.surgical = 1 THEN 'ScheduledSurgical'
      WHEN adm.admission_type <> 'ELECTIVE'
      AND sf.surgical = 1 THEN 'UnscheduledSurgical'
      ELSE 'Medical'
    END AS admissiontype
  FROM icustays AS ie
    INNER JOIN admissions AS adm ON ie.hadm_id = adm.hadm_id
    LEFT JOIN age AS va ON ie.hadm_id = va.hadm_id
    INNER JOIN co ON ie.stay_id = co.stay_id
    /* join to above views */
    LEFT JOIN pafi2 AS pf ON ie.stay_id = pf.stay_id
    LEFT JOIN surgflag AS sf ON adm.hadm_id = sf.hadm_id
    AND sf.serviceorder = 1
    LEFT JOIN comorb ON ie.hadm_id = comorb.hadm_id
    /* join to custom tables to get more data.... */
    LEFT JOIN gcs AS gcs ON ie.stay_id = gcs.stay_id
    LEFT JOIN vital ON ie.stay_id = vital.stay_id
    LEFT JOIN uo ON ie.stay_id = uo.stay_id
