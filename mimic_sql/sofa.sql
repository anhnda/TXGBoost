...
WITH co AS (
  ...
  FROM icustay_hourly AS ih
    INNER JOIN icustays AS ie ON ih.stay_id = ie.stay_id
),
pafi AS (
  ...
  SELECT ie.stay_id,
    ...
    CASE
      WHEN NOT vd.stay_id IS NULL THEN pao2fio2ratio
      ELSE NULL
    END AS pao2fio2ratio_vent
  FROM icustays AS ie
    INNER JOIN bg AS bg ON ie.subject_id = bg.subject_id
    LEFT JOIN ventilation AS vd ON ie.stay_id = vd.stay_id
...
vs AS (
  ...
  FROM co
    LEFT JOIN vitalsign AS vs ON co.stay_id = vs.stay_id
...
bili AS (
  ...
  FROM co
    LEFT JOIN enzyme AS enz ON co.hadm_id = enz.hadm_id
...
cr AS (
  ...
  FROM co
    LEFT JOIN chemistry AS chem ON co.hadm_id = chem.hadm_id
...
plt AS (
  ...
  FROM co
    LEFT JOIN complete_blood_count AS cbc ON co.hadm_id = cbc.hadm_id
...
uo AS (
  SELECT co.stay_id,
    ...
    MAX(
      CASE
        WHEN uo.uo_tm_24hr >= 22
        AND uo.uo_tm_24hr <= 30 THEN CAST(uo.urineoutput_24hr AS DOUBLE PRECISION) / uo.uo_tm_24hr * 24
      END
    ) AS uo_24hr
  FROM co
    LEFT JOIN urine_output_rate AS uo ON co.stay_id = uo.stay_id
...
vaso AS (
  ...
  FROM co
    LEFT JOIN epinephrine AS epi ON co.stay_id = epi.stay_id
    AND co.endtime > epi.starttime
    AND co.endtime <= epi.endtime
    LEFT JOIN norepinephrine AS nor ON co.stay_id = nor.stay_id
    AND co.endtime > nor.starttime
    AND co.endtime <= nor.endtime
    LEFT JOIN dopamine AS dop ON co.stay_id = dop.stay_id
    AND co.endtime > dop.starttime
    AND co.endtime <= dop.endtime
    LEFT JOIN dobutamine AS dob ON co.stay_id = dob.stay_id
    AND co.endtime > dob.starttime
    AND co.endtime <= dob.endtime
  WHERE NOT epi.stay_id IS NULL
...
scorecomp AS (
  SELECT co.stay_id,
    ...
    plt.platelet_min
  FROM co
    LEFT JOIN vs ON co.stay_id = vs.stay_id
    AND co.hr = vs.hr
    LEFT JOIN gcs ON co.stay_id = gcs.stay_id
    AND co.hr = gcs.hr
    LEFT JOIN bili ON co.stay_id = bili.stay_id
    AND co.hr = bili.hr
    LEFT JOIN cr ON co.stay_id = cr.stay_id
    AND co.hr = cr.hr
    LEFT JOIN plt ON co.stay_id = plt.stay_id
    AND co.hr = plt.hr
    LEFT JOIN pf ON co.stay_id = pf.stay_id
    AND co.hr = pf.hr
    LEFT JOIN uo ON co.stay_id = uo.stay_id
    AND co.hr = uo.hr
    LEFT JOIN vaso ON co.stay_id = vaso.stay_id
    AND co.hr = vaso.hr
),
scorecalc AS (
...
score_final AS (
  ...
  FROM scorecalc AS s WINDOW w AS (
      PARTITION BY stay_id
      ORDER BY hr NULLS FIRST ROWS BETWEEN 23 PRECEDING AND 0 FOLLOWING
    )
)
SELECT *
FROM score_final
