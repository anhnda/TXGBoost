...
WITH surgflag AS (
  ...
  FROM icustays AS ie
    LEFT JOIN services AS se ON ie.hadm_id = se.hadm_id
...
vent AS (
  SELECT ie.stay_id,
    MAX(
      CASE
        WHEN NOT v.stay_id IS NULL THEN 1
        ELSE 0
      END
    ) AS vent
  FROM icustays AS ie
    LEFT JOIN ventilation AS v ON ie.stay_id = v.stay_id
...
cohort AS (
  SELECT ie.subject_id,
    ...
    CASE
      ...
      WHEN adm.deathtime <= ie.intime THEN 1
      WHEN adm.dischtime <= ie.outtime
      AND adm.discharge_location = 'DEAD/EXPIRED' THEN 1
      ELSE 0
    END AS icustay_expire_flag,
    adm.hospital_expire_flag
  FROM icustays AS ie
    INNER JOIN admissions AS adm ON ie.hadm_id = adm.hadm_id
    INNER JOIN patients AS pat ON ie.subject_id = pat.subject_id
    LEFT JOIN age AS ag ON ie.hadm_id = ag.hadm_id
    LEFT JOIN surgflag AS sf ON ie.stay_id = sf.stay_id
    /* join to custom tables to get more data.... */
    LEFT JOIN first_day_gcs AS gcs ON ie.stay_id = gcs.stay_id
    LEFT JOIN first_day_vitalsign AS vital ON ie.stay_id = vital.stay_id
    LEFT JOIN first_day_urine_output AS uo ON ie.stay_id = uo.stay_id
...
scorecomp AS (
  SELECT co.subject_id,
    ...
    CASE
      ...
      AND heart_rate_max <= 125 THEN 3
      WHEN heart_rate_max >= 89
      AND heart_rate_max <= 106 THEN 1
      ELSE 0
    ...
    CASE
      ...
      WHEN heart_rate_max >= 107
      AND heart_rate_max <= 125 THEN heart_rate_max
      WHEN heart_rate_max >= 89
      AND heart_rate_max <= 106 THEN heart_rate_max
      ELSE CAST(
        (heart_rate_min + heart_rate_max) AS DOUBLE PRECISION
      ) / 2
    END AS heartrate,
    CASE
      WHEN mbp_min IS NULL THEN NULL
      ...
      WHEN mbp_min >= 51
      AND mbp_min < 61.33 THEN mbp_min
      ELSE CAST(
        (mbp_min + mbp_max) AS DOUBLE PRECISION
      ) / 2
    END AS meanbp,
    CASE
      WHEN resp_rate_min IS NULL THEN NULL
      ...
      ELSE CAST(
        (resp_rate_min + resp_rate_max) AS DOUBLE PRECISION
      ) / 2
    END AS resprate,
    CASE
      WHEN temperature_max IS NULL THEN NULL
      ...
      WHEN temperature_min >= 33.22
      AND temperature_min <= 35.93 THEN temperature_min
      WHEN temperature_max >= 33.22
      AND temperature_max <= 35.93 THEN temperature_max
      WHEN temperature_min < 33.22 THEN temperature_min
      WHEN temperature_min > 35.93
      AND temperature_min <= 36.39 THEN temperature_min
      WHEN temperature_max >= 36.89
      AND temperature_max <= 39.88 THEN temperature_max
      ELSE CAST(
        (
