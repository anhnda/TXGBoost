...
WITH cr AS (
    ...
    FROM icustays AS ie
        LEFT JOIN labevents AS le ON ie.subject_id = le.subject_id
        AND le.itemid = 50912
...
cr48 AS (
    ...
    FROM cr
        ...
        LEFT JOIN cr AS cr48 ON cr.stay_id = cr48.stay_id
...
cr7 AS (
    ...
    FROM cr
        ...
        LEFT JOIN cr AS cr7 ON cr.stay_id = cr7.stay_id
...
FROM cr
    LEFT JOIN cr48 ON cr.stay_id = cr48.stay_id
    AND cr.charttime = cr48.charttime
    LEFT JOIN cr7 ON cr.stay_id = cr7.stay_id
