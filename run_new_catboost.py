...
class SoftDecisionTree(nn.Module):
    ...
    - Explicit tree structure mimics CatBoost's decision trees
...
def main(data_filepath):
    """
    Run TBoostv2 pipeline with new data format using CatBoost.

    Args:
        data_filepath: Path to joblib file containing data in new format
    """
    print("="*80)
    print("TBoostv3 WITH NEW DATA FORMAT (CatBoost)")
    print("="*80)
    ...
    for fold, (train_full, test_p) in enumerate(trainTestPatients(patients, seed=xseed)):
        ...
        # Stage 3: Train CatBoost
        print("\n  [Stage 3] Training CatBoost...")
        ...
        # Baseline model
        print("\n  [Baseline] Training standard CatBoost...")
    ...
    ax1.set_title("Enhanced TBoostv2 with CatBoost (New Data Format)")
    ax2.set_title("Baseline CatBoost (New Data Format)")

    plt.tight_layout()
    plt.savefig("result/tboostv2_new_data_vs_baseline_catboost.png", dpi=300)
    print("\nPlot saved to: result/tboostv2_new_data_vs_baseline_catboost.png")
...
if __name__ == "__main__":
    # if len(sys.argv) < 2:
    #     print("\nUsage: python run_new_catboost.py <path_to_joblib_file>")
    #     print("\nExample:")
    #     print("  python run_new_catboost.py data/patients_new_format.joblib")
    #     sys.exit(1)
