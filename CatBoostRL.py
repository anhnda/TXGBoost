"""
REINFORCEMENT LEARNING VERSION: RNN Policy Network → CatBoost Judge
...
3. CatBoost provides reward signal (cannot backprop, so use policy gradient)
4. Train RNN policy to maximize expected reward

Key Features:
- Policy Gradient (REINFORCE algorithm)
- CatBoost as non-differentiable reward function
- Stochastic policy with exploration
- Validation to prevent overfitting

Differences from XGRL.py:
- Uses CatBoost instead of XGBoost
- CatBoost handles categorical features natively
- Different hyperparameters optimized for CatBoost
"""
...
# ==============================================================================
# 5. Reinforcement Learning Training Loop with CatBoost
# ==============================================================================
...
def train_policy_with_catboost_reward(
    ...
    """
    Train policy network using CatBoost as reward function.

    Algorithm:
    1. Sample Z from policy network
    2. Train CatBoost on [Static + Last + Z] → Prediction
    3. Compute reward based on CatBoost performance
    ...
    print("  [RL Training] Using CatBoost as Reward Judge...")
    ...
    for epoch in range(epochs):
        ...
        # =====================================================================
        # Step 2: Train/Update CatBoost periodically
        # =====================================================================
        ...
        # =====================================================================
        # Reward = 1 if CatBoost predicts correctly, 0 otherwise
        ...
        if (epoch + 1) % 5 == 0:
            ...
            with torch.no_grad():
                ...
                # Retrain CatBoost on deterministic features
                catboost_val_model = CatBoostClassifier(**catboost_params)
                catboost_val_model.fit(X_train, y_train, verbose=False)

                y_val_proba = catboost_val_model.predict_proba(X_val)[:, 1]
...
def main():
    print("="*80)
    print("RL POLICY (RNN + CatBoost) vs BASELINE (Last + Static)")
    print("="*80)
    ...
    for fold, (train_full, test_p) in enumerate(trainTestPatients(patients,seed=xseed)):
        ...
        # Train final CatBoost on deterministic features
