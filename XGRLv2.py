"""
REINFORCEMENT LEARNING V2: RNN Policy Network → XGBoost Judge

Goal: Beat baseline on BOTH AUC and AUPR using XGBoost instead of TabPFN
...
Differences from TabPFNRL:
- Uses XGBoost instead of TabPFN as the classifier
- No dataset size limitations (unlike TabPFN's 1024 sample limit)
...
# ==============================================================================
# 6. Conservative RL Training with XGBoost
# ==============================================================================
...
def train_policy_conservative_rl(
    ...
    Uses XGBoost instead of TabPFN
    """
    ...
    print("  [Conservative RL] AUPR-focused training with XGBoost...")
    ...
    for epoch in range(epochs):
        ...
        y_val_proba = xgb_model.predict_proba(X_val_stoch)[:, 1]
...
def main():
    print("="*80)
    print("RL POLICY V2 (Enhanced) with XGBoost")
    print("="*80)
    ...
    ax1.set_title("RL Policy + XGBoost", fontsize=14, fontweight='bold')
    ax1.legend(loc="lower right")
    ax1.grid(alpha=0.3)

    plt.tight_layout()
    plt.savefig("result/xgrl_v2.png", dpi=300)
    print("\nPlot saved to result/xgrl_v2.png")
